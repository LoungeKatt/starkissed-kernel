...
#define BGAP_THRESHOLD_T_COLD		67000	/* 67 deg C */
...
#ifdef CONFIG_TEMP_CONTROL
	if (temp_limit > 0) {
	  cold_threshold = temp_limit - 3000;
...
static void throttle_delayed_work_fn(struct work_struct *work)
{
	...
	if (curr >= BGAP_THRESHOLD_T_HOT || curr < 0) {
        ...
        if (throttle_enabled) {
            ...
            pr_info("[franciscofranco] %p - OMAP temp read %d exceeds the threshold but throttling is disabled.", __func__, curr);
        }
